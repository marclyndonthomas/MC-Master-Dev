...
- **Two simulation modes** (this dev/master copy keeps both; the GC and IFA variants ship only the first)
  - *Variable return* — each simulated path draws its own random monthly returns, so results carry both
    an uncertain realised average return and sequence-of-returns risk.
  - *Same average return* — every path's returns are shifted so its geometric mean equals the expected
    return exactly, holding the average fixed to isolate sequence-of-returns risk.

  Labels throughout the UI are worded for advisers explaining a chart to a client rather than in
  statistical terms ("average %/yr" not "arith. mean", "fixed return" not "linear", no σ symbol).
  Keep that register when adding UI here, since the variants are forked from this repo.
